-- Add payment & change-tracking columns to orders
alter table orders
  add column if not exists amount_received numeric default 0,
  add column if not exists change_amount   numeric default 0,
  add column if not exists change_given    boolean default true;

-- Update place_order RPC to accept optional payment params
create or replace function place_order(
  p_items            jsonb,
  p_amount_received  numeric default 0,
  p_change_amount    numeric default 0,
  p_change_given     boolean default true
)
returns jsonb
language plpgsql
security invoker
...
begin
  ...
  -- Insert order row with payment info
